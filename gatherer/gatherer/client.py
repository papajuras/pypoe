"""PoE Trade API client — SyncClient (mutex+backoff)."""
...
class TradeClient:
    ...
    @property
    def session(self):
        return self._sync._session
    @session.setter
    def session(self, s):
        self._sync._session = s
    ...
    def metrics(self) -> dict:
        now = time.time()
        return {
            "rate_limits": list(self._sync._rate_limits.values()),
            "backoff_remaining": max(0.0, self._sync._backoff_until - now),
            "delay": self._sync._delay,
            "min_gap": self._sync._min_gap,
        }
...
class SyncClient:
    def __init__(self, user_agent: str):
        ...
        self._jitter_fn = lambda: random.uniform(0.7, 1.8)
    ...
    def _rebuild_session(self):
        # curl_cffi caches DNS results (incl. failures) in the session's shared
        # cache; a failed lookup during VPN startup can wedge the process forever.
        # Fresh session = fresh DNS cache.
    ...
    def _send(self, method: str, url: str, data: dict | None):
        # Transient DNS failures (e.g. VPN netns still coming up after a deploy)
        # must not wedge the process via the cached failure — rebuild + retry once.
        try:
            if method == "POST":
                return self._session.post(url, json=data, timeout=30)
            return self._session.get(url, timeout=30)
        except DNSError:
            _audit("DNS  lookup failed — rebuilding session and retrying")
            logger.warning("DNS failure — rebuilding curl session")
            self._rebuild_session()
            if method == "POST":
                return self._session.post(url, json=data, timeout=30)
            return self._session.get(url, timeout=30)
        ...
        for rule_name in rules_str.split(","):
            ...
            for i, tier in enumerate(limit_tiers):
                ...
                if a > 0:
                    ...
                    if used > 0:
                        usage = used / a
                        tier_gap = resp_rate * (0.35 + usage * 1.3)
                    else:
                        tier_gap = resp_rate * 0.2
