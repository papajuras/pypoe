...
def save_text_for(entry: dict) -> str:
    ...
    if entry.get("influence") or entry.get("ambiguous"):
...
def load_affixes(item_type_name: str, cluster_implicit_type: str = "") -> tuple[list[dict], list[dict]]:
    ...
    for mod_id, m in mods.items():
        ...
        if influence or is_cluster:
            entry["search_text"] = get_search_text(stat_ids, raw_stats) or ", ".join(stat_lines[:2])
        raw_by_id[mod_id] = raw_stats
    ...
    if not is_cluster:
        _mark_ambiguous(prefixes, raw_by_id)
        _mark_ambiguous(suffixes, raw_by_id)
...
def _mark_ambiguous(side: list[dict], raw_by_id: dict[str, list[dict]]) -> None:
    """Flag entries whose name is shared (tier collisions) so they match by stat range."""
    counts: dict[str, int] = {}
    for e in side:
    ...
    for e in side:
        if counts[e["name"]] > 1:
            e["ambiguous"] = True
            if not e.get("search_text"):
                raw = raw_by_id.get(e["mod_id"], [])
                e["search_text"] = get_search_text(e["stat_ids"], raw) or ", ".join(e["stats"][:2])
...
def _search_text(stat_ids: tuple[str, ...], stat_ranges: tuple[tuple[str, int, int], ...]) -> str | None:
    """Cached core of get_search_text; args are hashable tuples."""
    translations = _load_translations()
    result = _find_translation(list(stat_ids), translations)
    if result is None:
        return None
    entry, start, count = result
    parts = re.split(r"\{[0-9]+\}", entry["English"][0]["string"])
    if not parts:
        return None
    tail = parts[-1]
    last_idx = start + count - 1
    last = stat_ranges[last_idx] if stat_ranges and 0 <= last_idx < len(stat_ranges) else None
    if last and last[1] != last[2]:
        return f"({last[1]}-{last[2]}){tail}".rstrip()
    return tail.strip() or None


def get_search_text(stat_ids: list[str], stats: list[dict] | None = None) -> str | None:
    """Tier-specific pattern: translation tail prefixed with the last stat's range.

    The in-game item text shows a rolled value followed by its tier range in parens
    (e.g. ``12(9-12)% increased Strength``), so ``(min-max)`` + tail is a literal
    substring that distinguishes tiers sharing a name.
    """
    ranges = tuple((s["id"], s["min"], s["max"]) for s in stats) if stats else ()
    return _search_text(tuple(stat_ids), ranges)
