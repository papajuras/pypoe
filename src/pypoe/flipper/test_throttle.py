...
from pypoe.flipper.client import TradeClient
...
class TestStagger(unittest.TestCase):
    ...
    def test_headers_set_delay(self):
        """Account: 3:5:60, no state → _delay = 5/3 × 0.3 + 1 = 1.5."""
        resp = FakeResponse()
        resp.headers = mk_headers("Account", Account="3:5:60")
        self.session.set_last(resp)
        self.client.search({})
        self.assertAlmostEqual(self.client._delay, 5 / 3 * 0.3 + 1, places=2)
        self.assertEqual(self.client._min_gap, 0.0)

    # ── 3. multi_tier_picks_restrictive ─────────────────────────

    def test_multi_tier_picks_restrictive(self):
        """Ip: 8:10:60,15:60:120,60:300:1800, no state → _delay = 300/60 × 0.3 + 1 = 2.5."""
        resp = FakeResponse()
        resp.headers = mk_headers("Ip", Ip="8:10:60,15:60:120,60:300:1800")
        self.session.set_last(resp)
        self.client.search({})
        self.assertAlmostEqual(self.client._delay, 300 / 60 * 0.3 + 1, places=2)

    # ── 4. multiple_rules_takes_max ─────────────────────────────

    def test_multiple_rules_takes_max(self):
        """Account: 3:5:60 + Ip: 60:300:1800, no state → _delay = max(1.667×0.3, 5.0×0.3) + 1 = 2.5."""
        resp = FakeResponse()
        resp.headers = mk_headers("Account,Ip", Account="3:5:60", Ip="60:300:1800")
        self.session.set_last(resp)
        self.client.search({})
        self.assertAlmostEqual(self.client._delay, max(5 / 3 * 0.3, 300 / 60 * 0.3) + 1, places=2)
    ...
    def test_delay_updates_regardless_of_previous(self):
        ...
        resp1 = FakeResponse()
        resp1.headers = mk_headers("Ip", Ip="60:300:1800", IpState="30:300:0")  # 50% usage → 7.25
        resp2 = FakeResponse()
        resp2.headers = mk_headers("Ip", Ip="60:300:1800", IpState="3:300:0")   # 5% usage → 4.375
    ...
    def test_buffer_always_added(self):
        """Header 1:1:1, no state → delay = 1/1 × 0.3 + 1 = 1.3."""
        resp = FakeResponse()
        resp.headers = mk_headers("Account", Account="1:1:1")
        self.session.set_last(resp)
        self.client.search({})
        self.assertAlmostEqual(self.client._delay, 1.3, places=2)
    ...
    def test_429_poe_exits(self):
        ...
        self.session.set_last(resp)
        with patch("pypoe.flipper.client.os._exit", side_effect=SystemExit(1)) as mock_exit:
    ...
    def test_429_cloudflare_continues(self):
        ...
        self.client._last = time.time() - 10
        self.session.set_last(resp)
        with patch("pypoe.flipper.client.os._exit") as mock_exit:
            t0 = time.monotonic()
    ...
    def test_delay_adapts_across_sequence(self):
        """6 requests: gaps [3.0, 3.0, 3.5, 3.5, 4.5] with dynamic delay."""
        clock = ControlledClock()

        with (patch('flipper.client.time.time', clock.time),
              patch('flipper.client.time.sleep', clock.sleep)):
            ...
            # Req 3: response has Account=3:5:60 (no state) → _delay=1.5 after response, but _enforce still uses min_gap
            resp = FakeResponse()
            resp.headers = mk_headers("Account", Account="3:5:60")
            self.session.set_last(resp)
            self.client.search({})
            ...
            # Req 4: _delay=1.5 takes effect → gap = 1.5 + 2.0 = 3.5
            resp = FakeResponse()
            resp.headers = mk_headers("Account", Account="3:5:60")
            self.session.set_last(resp)
            self.client.search({})
            ...
            # Req 5: _delay=1.5 → gap = 3.5
            #        response sets Ip=60:300:1800 (no state) → _delay=2.5
            resp = FakeResponse()
            ...
            # Req 6: _delay=2.5 takes effect → gap = 2.5 + 2.0 = 4.5
            resp = FakeResponse()
            resp.headers = mk_headers("Ip", Ip="60:300:1800")
            self.session.set_last(resp)
            self.client.search({})
        ...
        expected = [3.0, 3.0, 3.5, 3.5, 4.5]

        print(f"\n  Sequence gaps: {gaps}")

        for i, (actual, exp) in enumerate(zip(gaps, expected)):
            self.assertAlmostEqual(actual, exp, delta=0.01,
                msg=f"gap[{i}]: expected {exp}, got {actual}")

        self.assertAlmostEqual(self.client._delay, 300 / 60 * 0.3 + 1, places=1)
        self.assertEqual(self.client._min_gap, 0.0)
    ...
    def test_empty_state_low_delay(self):
        """No state data → 0.3x multiplier → _delay = 5.0 × 0.3 + 1 = 2.5."""
        resp = FakeResponse()
        resp.headers = mk_headers("Ip", Ip="60:300:1800")
        self.session.set_last(resp)
        self.client.search({})
    ...
    def test_multi_tier_state_takes_max(self):
        ...
        self.session.set_last(resp)
        self.client.search({})
        # Account: 5/3 × (0.5 + 0.667 × 1.5) = 1.667 × 1.5 = 2.5
        # Ip: 60/15 × (0.5 + 0.2 × 1.5) = 4.0 × 0.8 = 3.2
        # max = 3.2, + 1 = 4.2
        expected = 4.2
